# script_AttributeRelative
# Convert Absolute Values Into Relative Ones (With Regards to Attributes)
...
# Iteration and Convertion Into Percentage (With Regards to Attributes)
...
for i in dictDataFrames.keys():
    exportPath = 'C:/GitHub/OpenDataCustomerAnalytics/demographicsEducation/canton/{0}/AttributeRelative/'.format(i[-2:])
    exportPathExist = os.path.exists(exportPath)
    if not exportPathExist:
        os.makedirs(exportPath)
    exportFile = 'C:/GitHub/OpenDataCustomerAnalytics/demographicsEducation/canton/{0}/AttributeRelative/{1}_demographicsEducation_AttributeRelative.csv'.format(i[-2:], i)
